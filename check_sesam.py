...
OK = ['IN_QUEUE', 'ACTIVE', 'SUCCESSFUL', 'INFO',  'TIMING', 'INITIALISATION']
WARNING = ['CANCELLED', 'PARTIALLY_DELETED']
CRITICAL = ['ERROR']
UNKOWN = ['NONE']
exit_code = 0
...
def main():
    parser = argparse.ArgumentParser(description='Monitoring check for SEP Sesam backup states')
    ...
    try:
        ...
        backup_results = requests.post('http://' + args.hostname + ':' + args.port + '/sep/api/v2/backups/find', json={"dateFlagToday":"true"}, headers=headers)
